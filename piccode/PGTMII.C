...
{//auto char h;
    ...
    // Speed
    __CONFIG( HS & WDTDIS & LVPDIS & MCLRDIS & BORDIS & PWRTEN);
	GIE=0;
    //--------------------------------------------------------------------------
...
//	vertical=EEPROM_READ(41);
	...
	if ((count.msg==BAUD_RATE_19200) || (count.msg==BAUD_RATE_9600)) SPBRG=count.msg; else  SPBRG=BAUD_RATE_19200;
 //  SPBRG=count.msg;
 ...
 //SPBRG=count.msg;
  ...
  //  half_ra_bit=0;
    ...
    do {
			if  (half_ra_bit ) {RB3=0;frev = frev | 0x08;} else {RB3=1;frev = frev & 0xf7;};
	//	if (RBIF) {pec=count.ra_enc_count ;RBIF=0;}
//		if (dec_period_counter>=0){
       if ((TMR1IF) && (ra.ra_dir != 0)) {    // si completado periodo AR procesa
          //pasAR=0;                       // siguiente paso calculo de
            ...
            //TMR1ON = 1;
/*            ra_mstp+=ra.ra_dir;
	    ra_enc_count+=ra.ra_dir;
	  if (ra_enc_count<0) ra_enc_count=ra_encoder_res ;else if( ra_enc_count>ra_encoder_res) ra_enc_count=0;*/
	...
	if (ra.ra_dir==1)
		...
		//	if ((ra_pwm_value) <21 && (ra_pwm_value>0)) ra_pwm_value=21;
			...
			if (half_ra_bit){
			if (ra_mstp<MIDSTEP_RA+1) PORTA=sm ; else PORTA=sm1;}
