...
#define GET_TICKS 29
      ...
      if (RCIF) {
			...
			//0 ID 1 pedir posicion 3 byte
                case COUNTERS_GET_CMD: case M1_GET_SPEED_CMD: case GET_TICKS :
                     state = 0;
                  ...
                  // pointer = (char *) &ltemp;
					...
					case GET_TICKS :send=10; ltemp=count.time; pointer=(char *)&ltemp; break;};
			...
			//0 ID 12	 Correción activa backslash auxiliar.(byte) 4 byte
                case M1_BACKSLASH_MODE: case M2_BACKSLASH_MODE: case BAUDRATE_SET: case PRESCALER_SET: case M1_SLEW_CMD_SET:
					state=16;
                     break;
