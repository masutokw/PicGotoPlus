...
// 0 			6 					Invertir secuencia motor
// declinacion para cambio de orientacion
// 0 			7 					Secuencia original  secuencia
// motor declinacion
// 0 			8 		(#char#char)Cambio de valor de micro paso
// 0 			9 		(#int#char) Cambio de velocidad base para
// marchas LX200
// 0 			10 					Escribe en EEPROM matriz de
// velocidades
...
if (RCIF)
{
    ...
    case 1:
        ...
        case GET_TICKS :
            ...
            {
            case COUNTERS_GET_CMD:
                pointer= (char *) &count.ra_enc_count ;
                send = 1;
                break;
            case M1_GET_SPEED_CMD :
                send=10;
                pointer=(char*) & ra.ra_current_mstep_period;
                break;
            case GET_TICKS :
                send=10;
                ltemp=count.time;
            ...
            /*if (1) {pointer= (char *) &count.ra_enc_count ; send = 1;}
            else  {send=10; pointer=(char*) & count.time;};
        ...
        case M1_BACKSLASH_MODE:
        case M2_BACKSLASH_MODE:
        case BAUDRATE_SET:
        case PRESCALER_SET:
        case M1_SLEW_CMD_SET:
            state=16;
            break;
    ...
    case 3:             // byte alto parametro
        *pointer2 = buffer;
        ...
        case M2_SPEED_SET_CMD:
            ...
            {
                dec_dir = -1;
                dec_current_mstep_period = -dec_current_mstep_period;
            }
            else
            {
                dec_dir = 0;
    ...
    case 12:
        ...
        else if (buffer <8 )speeds.dec_speeds[buffer - 6] = speedbuff;
        state = 0;
        break;
    ...
    case 15:
        ...
        else if (ra.vdpost>0)	ra.ra_dir=1;
        else ra.ra_dir=0;
    ...
    case CHECK_STATE:
        ...
        {
            if (buffer== CMD_FCODE) state=SELECT_STATE;
        };
        break;
